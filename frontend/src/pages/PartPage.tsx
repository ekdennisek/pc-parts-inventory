...
export const PartPage: React.FC = () => {
    ...
    const setSortOption = useCallback(
        (value: SortOption) =>
            setSearchParams((prev) => setParam(prev, "sort", value === "standard" ? null : value), {
                replace: true,
            }),
        [setSearchParams],
    );
    ...
    const setSelectedFilters = useCallback(
        (values: string[]) =>
            setSearchParams((prev) => setParam(prev, "filter", values.length > 0 ? values : null), {
                replace: true,
            }),
        [setSearchParams],
    );
    ...
    const yearOptions = useMemo(() => {
        const years = parts.filter((p) => p.releaseYear !== undefined).map((p) => p.releaseYear!);
